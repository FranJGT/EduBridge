...
const TAB_ICONS: Record<TabId, (a: boolean) => React.ReactNode> = {
  home: (a) => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill={a ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export function BottomTabBar({ active, onTabChange }: BottomTabBarProps) {
  ...
  return (
    <nav aria-label="Main navigation" className="fixed bottom-0 left-0 right-0 z-40 flex justify-center px-5 pb-5 pt-3 bg-gradient-to-t from-background via-background to-transparent">
      <div role="tablist" className="flex w-full max-w-md h-14 rounded-2xl bg-card border border-border shadow-lg overflow-hidden p-1">
        {(["home", "journey", "scan", "progress"] as TabId[]).map((tab) => {
          ...
          return (
            <motion.button
              ...
              role="tab"
              className={`flex-1 flex flex-col items-center justify-center gap-0.5 rounded-xl transition-colors focus-visible:ring-2 focus-visible:ring-primary ${
